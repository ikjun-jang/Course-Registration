...
public class CourseService {
	 ...
	 public boolean deleteRegdCourse(String userId, int courseNum){
		 ...
		 String sql = "delete from registeredcourses where student_id = :studentId" +
			  ...
			  sql = "delete from studentrequests where student_id = :studentId" +
				    " and course_number = :courseNum";
	 ...
	 public String registerCourses(String userId, Integer courseNumber) {
	  ...
	  boolean hasPreReq = checkPrerequisites(userId, courseNumber);
	  System.out.println("Has Prereq results"+hasPreReq);
	  if (hasPreReq){
		  dbRegStatus = "pending";
	  }
	  System.out.println(dbRegStatus);
	 ...
	 public String getRegistrationStatus(String userId, Integer courseNumber){
		 ...
		 RowMapper<Course> mapper1 = new RowMapper<Course>() { 
	         public Course mapRow(ResultSet rs, int rowNum) throws SQLException {
	        	...
	        	course.setCourseName(rs.getString("student_advisor"));
	            return course;
	          }};
	     ...
	     if(!coursePrefix.equals(programPrefix)){
	    	 ...
	    	 addStudentRequest(userId, courseNumber, "Registration request.", programPrefixList.get(0).getCourseName());
	    	 return "Registered for course outside " + programPrefix + "program. Waiting for advisors approval.";
	     }
		 return regStatus;
	 }
	 
	 public boolean checkPrerequisites(String userId, Integer courseNumber){
		 boolean preReqResult = false;
		 String sql = "select course_num, prereq_status from studenprereqs where course_num in (select prequisite_course_number from courseprerequisites where course_number="+courseNumber+")";
		 System.out.println("************Prereq query::"+sql);
		 
		 RowMapper<Course> mapper = new RowMapper<Course>() { 
	         public Course mapRow(ResultSet rs, int rowNum) throws SQLException {
	        	Course course = new Course();
	        	course.setCourseNumber(rs.getInt("course_num"));
	        	course.setCourseStatus(rs.getString("prereq_status"));
	            return course;
	          }};
	     List<Course>preReqList = jdbcTemplate.query(sql,mapper);
	     ...
	     if(preReqList.size() == 0){
	    	 ...
	    	 for(int i=0; i<preReqList.size(); i++){
	    		 String status = preReqList.get(i).getCourseStatus();
	     ...
	     if (preReqResult){
	    	 String advisorId = getAdvisorId(userId);
	    	 addStudentRequest(userId, courseNumber, "Registration request.", advisorId);
	     }
		 return preReqResult;
	 }
	 
	 public String getAdvisorId(String userId){
		 String sql = "select program_prefix, student_advisor from student, EnrolledPrograms, programs " + 
		    	   "where student.student_netid = EnrolledPrograms.student_id " + 
				   "and EnrolledPrograms.program_id = programs.program_id " +
				   "and student.student_netid = '"+ userId +"'";
			 RowMapper<Course> mapper1 = new RowMapper<Course>() { 
		     ...
		     List<Course>programPrefixList = jdbcTemplate.query(sql,mapper1);
		     return ""+programPrefixList.get(0).getCourseName();
	 }
	 
	 public int getNextRequestId(String advisorId){
		 int reqId = 0;
		 String sql = "select max(request_id) from studentrequests where advisor_id='"+advisorId+"'";
			 RowMapper<Course> mapper1 = new RowMapper<Course>() { 
		         public Course mapRow(ResultSet rs, int rowNum) throws SQLException {
		        	Course course = new Course();
		     ...
		     List<Course>requestId  = jdbcTemplate.query(sql,mapper1);
		     reqId = 1 + requestId.get(0).getCourseNumber();
		 return reqId;
	 }
	 
	 private void addStudentRequest(String userId, Integer courseNumber,
			String requestDesc, String advisorId) {
		// TODO Auto-generated method stub
		  logger.debug("Adding student request.");
		  //Random rgen = new Random();
		  int requestId = getNextRequestId(advisorId);
		  // Prepare our SQL statement
		  String sql = "insert into studentrequests values( " +
		    ":requestId, :studentId, :advisorId, :requestDesc, :courseNum, 'pending')";
